...
def generate_pdf_summary_report(data, safety_data, gri_status, chart_images):
    ...
    for std, info in gri_status.items():
        gri_table_data.append([std, info['status'], info['description']])
    ...
    if not recommendations:
        recommendations.append("✅ All metrics meet or exceed industry standards. Continue best practices.")
...
def generate_excel_summary_report(data, safety_data, gri_status):
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        kpi_df = pd.DataFrame([
            {"Metric": "CO₂ Emissions", "Value": data['co2'], "Unit": "metric tons", "GRI Standard": "GRI 305-1"},
            {"Metric": "Energy Consumption", "Value": data['energy'], "Unit": "MWh", "GRI Standard": "GRI 302-1"},
            {"Metric": "Water Usage", "Value": data['water'], "Unit": "m³", "GRI Standard": "GRI 303-3"},
            {"Metric": "Waste Generated", "Value": data['waste'], "Unit": "tons", "GRI Standard": "GRI 306-3"},
            {"Metric": "Renewable Energy", "Value": data['renewable'], "Unit": "%", "GRI Standard": "GRI 302-2"},
            {"Metric": "LTIFR", "Value": safety_data['ltifr'], "Unit": "rate", "GRI Standard": "GRI 403-9"},
            {"Metric": "Near Misses", "Value": safety_data['near_misses'], "Unit": "number", "GRI Standard": "GRI 403-9"},
        ])
        kpi_df.to_excel(writer, sheet_name="KPIs", index=False)
        safety_df = pd.DataFrame([
            {"Indicator": "Fatalities", "Value": safety_data['fatalities'], "Unit": "number"},
            {"Indicator": "Lost Time Injuries", "Value": safety_data['lost_time_injuries'], "Unit": "number"},
            {"Indicator": "Near Misses", "Value": safety_data['near_misses'], "Unit": "number"},
            {"Indicator": "Lost Work Days", "Value": safety_data['lost_days'], "Unit": "days"},
        ])
        safety_df.to_excel(writer, sheet_name="Safety", index=False)
        gri_df = pd.DataFrame([{"Standard": std, "Status": info['status'], "Description": info['description']} for std, info in gri_status.items()])
        gri_df.to_excel(writer, sheet_name="GRI Compliance", index=False)
        recommendations = []
        if safe_float(data['co2']) > 47000: recommendations.append("Reduce CO₂ emissions by 15%")
        if safe_float(data['renewable']) < 30: recommendations.append("Increase renewable energy share to 30%+")
        if safe_float(safety_data['ltifr']) > 1.5: recommendations.append("Reduce LTIFR")
        if safe_float(safety_data['near_misses']) > 50: recommendations.append("Investigate near misses")
        if not recommendations: recommendations.append("All metrics meet industry standards")
        rec_df = pd.DataFrame({"Recommendations": recommendations})
        rec_df.to_excel(writer, sheet_name="Recommendations", index=False)
        info_df = pd.DataFrame([
            {"Info": "Team Leader", "Value": "Ismail Kamal"},
            {"Info": "Supervisor", "Value": "Dr. Mohamed Tash"},
            {"Info": "Report Date", "Value": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        ])
...
def create_accidents_chart(safety_data):
    ...
    if not categories:
        categories = ['Fatalities','Lost Time Injuries','Near Misses']
        values = [0,0,0]
    fig = go.Figure(data=[go.Bar(x=categories, y=values, marker_color=['#D32F2F','#F57C00','#FFC107'][:len(categories)], text=values, textposition='outside')])
...
def generate_safety_analysis(safety_data):
    ...
    if safety_data['lost_time_injuries'] != "N/A":
        ...
        if lti_val > 10:
            analysis += f"⚠️ {int(lti_val)} Lost Time Injuries - Above acceptable range\n\n"
    ...
    if safety_data['near_misses'] != "N/A":
        ...
        if nm_val > 50:
            analysis += f"⚠️ High Near Misses ({int(nm_val)}) - Proactive safety culture needed\n\n"
    ...
    if safety_data['ltifr'] != "N/A":
        ...
        if ltifr_val > 2.0:
            analysis += f"⚠️ LTIFR: {ltifr_val} - Above industry average\n\n"
            ...
            analysis += f"✅ LTIFR: {ltifr_val} - Excellent performance!\n\n"
    if not recommendations:
        recommendations = ["✅ Continue current safety practices", "📊 Benchmark against best-in-class companies"]
...
if not st.session_state.comparison_mode:
    ...
    if file:
        ...
        if st.button("🔍 Analyze Report", type="primary", use_container_width=True):
            ...
            for i, (std, info) in enumerate(gri_status.items()):
                with [col1, col2, col3][i%3]:
                    color = "#2E7D32" if "✅" in info['status'] else "#D32F2F"
                    st.markdown(f"<div style='background:#F5F5F5; padding:12px; border-radius:10px; margin:5px; text-align:center;'><b>{std}</b><br><span style='color:{color}; font-size:18px;'>{info['status']}</span><br><span style='font-size:11px; color:gray;'>{info['description']}</span></div>", unsafe_allow_html=True)

            # Summary Metrics
            st.markdown("---")
            st.markdown("## 📈 Summary Metrics")
            col1, col2, col3, col4 = st.columns(4)
            with col1: st.metric("ESG Score", "77/100 (B+)", delta="+6 vs Industry")
            with col2: st.metric("GRI Compliance", "78%", delta="+8% vs Last Year")
            with col3: st.metric("Industry Rank", "Top 25%", delta="Improved")
...
""", unsafe_allow_html=True)        font-size: 36px;
        font-weight: 700;
    }
    .main-header p {
        color: #E8F5E9;
...
""", unsafe_allow_html=True)        background: linear-gradient(135deg, #f5f7fa 0%, #ffffff 100%);
    ...
    .team-table th { background: #2E7D32; color: white; padding: 12px; text-align: center; font-size: 16px; }
    .team-table td { padding: 10px; text-align: center; border-bottom: 1px solid #ddd; }
    .team-leader-row { background: linear-gradient(135deg, #FFF8E1 0%, #FFECB3 100%); font-weight: bold; }
    .team-leader-name { color: #D32F2F !important; font-weight: bold !important; font-size: 16px !important; }
    .team-member-name { color: #1565C0 !important; font-size: 15px !important; font-weight: 500 !important; }
    .supervisor-card {
        background: linear-gradient(135deg, #0D47A1 0%, #1B5E20 100%);
    ...
    .supervisor-title { color: #FFD54F; font-size: 22px; margin: 0 0 10px 0; }
    .supervisor-name { color: #FF0000; font-weight: bold; font-size: 36px; margin: 15px 0; text-shadow: 2px 2px 4px rgba(0,0,0,0.2); }
    .supervisor-qualification { font-size: 18px; color: white; font-weight: bold; }
    .stat-card {
        background: linear-gradient(135deg, #1B5E20 0%, #2E7D32 100%);
    ...
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #0A2E0F 0%, #0D47A1 100%); }
    .team-section { padding: 15px; border-radius: 15px; margin: 10px 0; }
    .team-title { color: #FFD54F; font-size: 20px; font-weight: bold; margin-bottom: 20px; text-align: center; border-bottom: 2px solid #FFD54F; padding-bottom: 8px; }
    .team-leader-name-side { color: #FFD54F !important; font-size: 22px !important; font-weight: bold !important; background: rgba(0,0,0,0.3); padding: 10px; border-radius: 10px; margin: 10px 0; text-align: center; }
    .team-member-name-side { color: #FFFFFF !important; font-size: 16px !important; font-weight: 500 !important; padding: 6px 10px; margin: 5px 0; background: rgba(255,255,255,0.15); border-radius: 8px; text-align: center; }
    .supervisor-section { background: linear-gradient(135deg, #FFEBEE 0%, #FFCDD2 100%); padding: 20px; border-radius: 15px; margin: 15px 0; text-align: center; }
    .supervisor-name-side { color: #FF0000 !important; font-size: 28px !important; font-weight: bold !important; margin: 10px 0; }
    .supervisor-title-side { color: #2E7D32 !important; font-size: 16px !important; font-weight: bold !important; }
    </style>
""", unsafe_allow_html=True)
...
users = {
    "admin": "1234",
    "ismail": "2024",
    "Dtash": "0000"   # المستخدم التجريبي العام
}
...
def generate_pdf_summary_report(data, safety_data, gri_status, chart_images):
    ...
    table = Table(kpi_table_data, colWidths=[130, 80, 80, 90])
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#1B5E20')),('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),('ALIGN',(0,0),(-1,-1),'CENTER'),('GRID',(0,0),(-1,-1),1,colors.grey),('FONTSIZE',(0,0),(-1,-1),10)]))
    story.append(table)
    story.append(Spacer(1, 20))
    
    # Safety Table
    story.append(Paragraph("🛡️ SAFETY PERFORMANCE", heading_style))
    safety_table_data = [['Indicator', 'Value', 'Unit'],['Fatalities', safety_data['fatalities'], 'number'],['Lost Time Injuries', safety_data['lost_time_injuries'], 'number'],['Near Misses', safety_data['near_misses'], 'number'],['Lost Work Days', safety_data['lost_days'], 'days']]
    safety_table = Table(safety_table_data, colWidths=[150,100,100])
    safety_table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#C62828')),('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),('ALIGN',(0,0),(-1,-1),'CENTER'),('GRID',(0,0),(-1,-1),1,colors.grey)]))
    story.append(safety_table)
    story.append(Spacer(1, 20))
    ...
    for std, info in gri_status.items():
        gri_table_data.append([std, info['status'], info['description']])
    gri_table = Table(gri_table_data, colWidths=[130,80,250])
    gri_table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#2E7D32')),('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),('ALIGN',(0,0),(-1,-1),'CENTER'),('GRID',(0,0),(-1,-1),1,colors.grey),('FONTSIZE',(0,0),(-1,-1),9)]))
    story.append(gri_table)
    story.append(Spacer(1, 20))
    
    # Add all charts (each on a new page or continuous)
    story.append(PageBreak())
    story.append(Paragraph("📈 ANALYSIS CHARTS", heading_style))
    charts_to_include = [
        ("Accidents & Near Misses", chart_images.get('accidents')),
...
def generate_excel_summary_report(data, safety_data, gri_status):
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        rec_df = pd.DataFrame({"Recommendations": recommendations})
        rec_df.to_excel(writer, sheet_name="Recommendations", index=False)
        info_df = pd.DataFrame([{"Info": "Team Leader", "Value": "Ismail Kamal"}, {"Info": "Supervisor", "Value": "Dr. Mohamed Tash"}, {"Info": "Report Date", "Value": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}])
        info_df.to_excel(writer, sheet_name="Report Info", index=False)
    return filename
...
def create_gauge_comparison_chart(value, metric_name, industry_avg, target):
    val = safe_float(value)
    fig = go.Figure(go.Indicator(mode="gauge+number+delta", value=val if val>0 else 50, title={"text": f"{metric_name}<br><span style='font-size:12px'>vs Industry Avg: {industry_avg}</span>", "font": {"size": 16}}, delta={"reference": industry_avg, "increasing": {"color": "#2E7D32"}, "decreasing": {"color": "#D32F2F"}}, gauge={"axis": {"range": [0, target*1.2]}, "bar": {"color": "#1B5E20"}, "steps": [{"range": [0, industry_avg], "color": "#FFCDD2"}, {"range": [industry_avg, target], "color": "#C8E6C9"}, {"range": [target, target*1.2], "color": "#FFF9C4"}], "threshold": {"line": {"color": "red", "width": 4}, "value": target}}))
    fig.update_layout(height=300, margin=dict(l=20, r=20, t=50, b=20))
    return fig

def create_bar_comparison_chart(current, industry_avg, best_in_class, metric_name, unit):
    current_val = safe_float(current)
    fig = go.Figure(data=[go.Bar(name='Your Company', x=[metric_name], y=[current_val], marker_color='#2E7D32'), go.Bar(name='Industry Average', x=[metric_name], y=[industry_avg], marker_color='#F57C00'), go.Bar(name='Best in Class', x=[metric_name], y=[best_in_class], marker_color='#1565C0')])
    fig.update_layout(title=f"{metric_name} Benchmarking", yaxis_title=unit, height=400, barmode='group')
    return fig
...
def create_esg_scorecard():
    ...
    company_scores = [78,72,82,70]
    industry_scores = [72,68,75,65]
    fig = go.Figure(data=[go.Bar(name='Your Company', x=categories, y=company_scores, marker_color='#2E7D32'), go.Bar(name='Industry Average', x=categories, y=industry_scores, marker_color='#F57C00')])
    fig.update_layout(title="ESG Scorecard - GRI Standards Benchmark", yaxis_title="Score (0-100)", height=400, barmode='group', yaxis_range=[0,100])
    return fig
...
def create_accidents_chart(safety_data):
    categories = []
    values = []
    if safety_data['fatalities'] != "N/A": categories.append('Fatalities'); values.append(safe_float(safety_data['fatalities']))
    if safety_data['lost_time_injuries'] != "N/A": categories.append('Lost Time Injuries'); values.append(safe_float(safety_data['lost_time_injuries']))
    if safety_data['near_misses'] != "N/A": categories.append('Near Misses'); values.append(safe_float(safety_data['near_misses']))
    if not categories: categories = ['Fatalities','Lost Time Injuries','Near Misses']; values = [0,0,0]
    fig = go.Figure(data=[go.Bar(x=categories, y=values, marker_color=['#D32F2F','#F57C00','#FFC107'][:len(categories)], text=values, textposition='outside')])
    fig.update_layout(title="Accidents & Near Misses Dashboard", xaxis_title="Incident Type", yaxis_title="Number of Incidents", height=400)
    return fig
...
def create_ltifr_gauge(safety_data):
    ltifr_val = safe_float(safety_data['ltifr']) if safety_data['ltifr'] != "N/A" else 1.2
    fig = go.Figure(go.Indicator(mode="gauge+number+delta", value=ltifr_val, title={"text": "LTIFR (Lost Time Injury Frequency Rate)<br><span style='font-size:12px'>Lower is Better</span>", "font": {"size": 16}}, delta={"reference": 1.5, "decreasing": {"color": "#2E7D32"}, "increasing": {"color": "#D32F2F"}}, gauge={"axis": {"range": [0,5]}, "bar": {"color": "#F57C00"}, "steps": [{"range": [0,1], "color": "#C8E6C9"}, {"range": [1,2], "color": "#FFF9C4"}, {"range": [2,5], "color": "#FFCDD2"}], "threshold": {"line": {"color": "red", "width": 4}, "value": 2.0}}))
    fig.update_layout(height=300)
    return fig
...
""", unsafe_allow_html=True)        box-shadow: 0 8px 25px rgba(0,0,0,0.1);
        border: 1px solid rgba(46,125,50,0.2);
    }
    .team-table-title {
        background: linear-gradient(135deg, #1B5E20 0%, #2E7D32 100%);
